  ...
  for (const [field, rule] of Object.entries(schema)) {
    ...
    if (rule.type === 'array' && Array.isArray(value) && rule.items) {
      for (let i = 0; i < value.length; i++) {
        const itemResult = validateRequest(
          { item: value[i] },
          { item: rule.items },
        )
        if (!itemResult.valid) {
          fieldErrors.push(
            `${field}[${i}] ${Object.values(itemResult.errors)[0]}`,
          )
        }
      }
    }
    ...
    if (rule.type === 'object' && typeof value === 'object' && rule.schema) {
      ...
      if (!nestedResult.valid) {
        for (const [nestedField, nestedErrors] of Object.entries(
          nestedResult.errors,
        )) {
          fieldErrors.push(
            `${field}.${nestedField}: ${nestedErrors.join(', ')}`,
          )
        }
      }
    }
