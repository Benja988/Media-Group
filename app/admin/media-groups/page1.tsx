...
// Fetch media groups with optional filters
async function fetchMediaGroups(status: string = 'active', limit: number = 1): Promise<MediaGroup[]> {
  try {
    ...
    const url = `${baseUrl}/api/media-groups?${params.toString()}`;
    ...
    if (!res.ok) {
      console.error('Failed to fetch media groups:', res.statusText);
      return [];
    }

    const data: MediaGroup[] = await res.json();
    return data;
  } catch (err) {
...
export default async function MediaGroupPage() {
  // Fetch active media groups
  const mediaGroups = await fetchMediaGroups('active', 1);
